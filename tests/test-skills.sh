...
test_skill_docs_exist() {
    echo ""; echo "Test: all 15 skill docs exist"
    local missing="" s
    for s in start health memory status deploy security testing costs rollback build-rules critical-thinker research evaluate review drift-check; do
        [ -f "$SKILLS/$s/$s.md" ] || missing="${missing:+$missing }$s"
    done
    [ -z "$missing" ] && pass "All 15 skill docs present" || fail "Missing skill docs: $missing"
}

test_agent_docs_exist() {
    echo ""; echo "Test: agent docs present for all three evaluators"
    local missing="" a
    for a in gate-evaluator spec-reviewer drift-evaluator; do
        [ -f "$PROJECT_DIR/.claude/agents/$a.md" ] || missing="${missing:+$missing }$a"
    done
    [ -z "$missing" ] && pass "Agent docs present (gate-evaluator, spec-reviewer, drift-evaluator)" || fail "Missing agent docs: $missing"
}

test_skill_docs_have_trigger() {
    echo ""; echo "Test: each skill doc declares a Trigger"
    local bad="" s
    for s in start deploy security testing costs rollback build-rules critical-thinker research evaluate review drift-check; do
...
test_all_docs_frontmatter() {
    echo ""; echo "Test: all 15 skill docs begin with YAML frontmatter"
    local bad="" s
    for s in start health memory status deploy security testing costs rollback build-rules critical-thinker research evaluate review drift-check; do
...
test_manifest_json() {
    echo ""; echo "Test: skill-manifest --json lists 15 skills, tokens_metadata numeric"
    local out n nums
    ...
    [ "$n" = "15" ] && [ "$nums" = "15" ] && pass "15 skills, all tokens_metadata numeric" || fail "n=$n nums=$nums"
}
